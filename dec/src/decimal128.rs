...
// use crate::InvalidPrecisionError;
use crate::Rounding;
...
impl Decimal128 {
    ...
    /// Inaccurate conversion to a 64 bit number
    pub fn to_float(self) -> f64 {
        let d: Decimal<12> = Decimal::from(self);
        d.try_into().expect("decimal to float conversion failed")
    }

    /// highly inaccurate conversion to a 64 bit number
    pub fn fast_float(self) -> f64 {
        ((-1.0_f64).powi(self.is_negative() as i32))
            * (self.coefficient() as f64)
            * 10.0_f64.powi(self.exponent())
    }

    /// Rounds up beneath the given exponent
    pub fn round_up(self, exp: isize) -> Decimal128 {
        const N: usize = 12;
        let mut cx = Context::<Decimal<N>>::default();
        let mut d: Decimal<N> = Decimal::from(self);
        let rhs: Decimal<N> = Decimal::from(exp);
        cx.set_rounding(Rounding::Up);
        cx.rescale(&mut d, &rhs);
        d.to_decimal128()
    }

    /// Fast round using float conversion, seems to actually work
    /// TODO
    pub fn conversion_round_up(self, dec_places: isize) -> Decimal128 {
        todo!();
    }

    /// Rounds down beneath the given exponent
    pub fn round_down(self, exp: isize) -> Decimal128 {
        const N: usize = 12;
        let mut cx = Context::<Decimal<N>>::default();
        let mut d: Decimal<N> = Decimal::from(self);
        let rhs: Decimal<N> = Decimal::from(exp);
    ...
    /// Gets square root of the value
    pub fn sqrt(self) -> Decimal128 {
        let mut d: Decimal<12> = Decimal::from(self);
        let mut ctx = Context::<Decimal<12>>::default();
        ctx.sqrt(&mut d);
        d.to_decimal128()
    }

    /// Fast, highly inaccurate square root
    pub fn fast_sqrt(self) -> Decimal128 {
        Decimal128::from(self.fast_float().sqrt())
    }
    ...
    /// Fast, highly inaccurate natural logarithm
    pub fn fast_ln(self) -> D128 {
        D128::from(self.fast_float().ln())
    }

    /// Gets absolute of the value
    ...
    /// Gets max
    pub fn max(self, rhs: Decimal128) -> Ordering {
        if self > rhs {
            Ordering::Greater
        } else if self < rhs {
            Ordering::Less
    ...
    /// Gets min
    pub fn min(self, rhs: Decimal128) -> Ordering {
        if self < rhs {
            Ordering::Less
        } else if self > rhs {
            Ordering::Greater
...
impl From<f64> for Decimal128 {
    fn from(n: f64) -> Decimal128 {
        let d: Decimal<12> = Decimal::from(n);
        d.to_decimal128()
    }
}
...
impl<'de> serde::de::Deserialize<'de> for Decimal128 {
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for MyVisitor {
            ...
            where
                E: serde::de::Error,
            {
                match Decimal128::from_str(val) {
                    Ok(d) => Ok(d),
                    Err(_) => Err(E::custom("failed to parse number")),
                }
            }
        }
